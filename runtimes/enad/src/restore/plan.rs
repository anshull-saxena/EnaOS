...
impl RestorePlanner {
    ...
    pub fn preview(&self, snapshot: &WorkspaceSnapshot) -> RestorePreview {
        ...
        for term in &snapshot.terminals {
            actions.push(RestoreActionPreview {
                label: format!("Restore terminal: {} ({})", term.app, term.working_directory.as_deref().unwrap_or("default")),
    ...
    pub fn plan_restoration(
            ...
            for ws in &snapshot.workspaces {
                if ws.is_focused {
                    let node = PlanNode::new(
                        ...
                        ActionType::SwitchWorkspace { workspace: ws.name.clone() },
                    );
                    let node_id = node.id;
                    nodes.push(node);

                    if let Some(prev) = prev_node_id {
                        edges.push(PlanEdge { from: prev, to: node_id, condition: EdgeCondition::Success });
                    }
        ...
        if use_selections.applications {
            ...
            for app in &snapshot.applications {
                let node = PlanNode::new(
                    ...
                    ActionType::OpenApp { app: app.name.clone() },
                );
                let node_id = node.id;
                nodes.push(node);
                ...
                if app.is_focused {
                    ...
                    edges.push(PlanEdge { from: node_id, to: focus_id, condition: EdgeCondition::Success });
        ...
        if use_selections.terminals {
            for term in &snapshot.terminals {
                ...
                let node = PlanNode::new(&label, ActionType::LaunchCommand {
                    command: if command.is_empty() { term.app.clone() } else { command },
                    args: Vec::new(),
                }).requires_approval();
        ...
        if use_selections.browser_urls {
            for tab in &snapshot.browser_urls {
                ...
                let node = PlanNode::new(&label, ActionType::OpenUrl { url: tab.url.clone() });
                let node_id = node.id;
                nodes.push(node);
        ...
        let parts: Vec<&str> = [
            if use_selections.applications { Some("apps") } else { None },
            if use_selections.workspaces { Some("workspaces") } else { None },
            if use_selections.terminals { Some("terminals") } else { None },
            if use_selections.browser_urls { Some("browser") } else { None },
        ...
        let has_terminal = plan.nodes.iter().any(|n| {
            matches!(&n.action, ActionType::LaunchCommand { .. })
        });
